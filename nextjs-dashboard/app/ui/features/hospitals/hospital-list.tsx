...
export function HospitalTable({ hospitals, pageSize, pending, canManage, onCreate, onView, onEdit, onDelete }: { hospitals: Hospital[]; pageSize: number; pending: boolean; canManage: boolean; onCreate: () => void; onView: (hospital: Hospital) => void; onEdit: (hospital: Hospital) => void; onDelete: (hospital: Hospital) => void }) {
  const { t } = useI18n();
  const columns: TableColumnsType<Hospital> = [
    ...
    { title: '', key: 'actions', width: 56, align: 'center', render: (_, hospital) => <HospitalActions hospital={hospital} canManage={canManage} onView={() => onView(hospital)} onEdit={() => onEdit(hospital)} onDelete={() => onDelete(hospital)} /> },
  ];
  return <Table<Hospital> rowKey="hospitalId" dataSource={hospitals} columns={columns} pagination={false} loading={pending} scroll={{ x: 720, ...(pageSize > 10 ? { y: 'min(60vh, 640px)' } : {}) }} sticky={pageSize > 10} locale={{ emptyText: <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description={t('No hospitals found')}>{canManage ? <Button type="primary" onClick={onCreate}>{t('Add the first hospital')}</Button> : null}</Empty> }} />;
}
